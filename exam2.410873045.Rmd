...
```{r ans13}
"臺?台?北?市?立?第?一女子?高?級?中?學?" -> regex_hs1st
"[臺台]?北市?立?第?一女子?高?級?中?學?" ->regex_hs1st
#北跟一女都要有
regex_hs1st
```
...
```{r}
data228$`檔號` %>% str_split("/") -> split_docCode
map(split_docCode,~.x[[1]]) ->list_doc
  unlist(list_doc)->docNumbers
data$`全宗名編號` <- factor(docNumbers)
map(split_docCode,~.x[[2]]) ->list_doc
  unlist(list_doc)->docNumbers
data$`全宗名編號` <- as.integer(docNumbers)
```
...
```{r ans22}
pick_note <- data228$`檔號`[data228$`附註`=="表冊名稱:台灣省二二八事變正法及死亡人犯名冊"]
pick_name <- data228$`個人名稱`[data228$`檔號`==
                              pick_note]
...
str(courseStructure)
regex_1 <- "[\u4e00-\u9FFF]+系[1-4]{1}"
majorGradeCombos <- str_extract(courseStructure$`應修系級`,regex_1)
